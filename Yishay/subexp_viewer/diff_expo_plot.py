def draw(*args):
    ...
    if type(args[0]) != str:
        ...
        path_fits = '/Users/sarelg/Documents/NLR/new_fits/sdss/files/spec-%s-%d-%s.fits' % (plate, mjd, fiber)
    else:
    ...
    for i in range(len(data)):
        plt.step(data[i][0], data[i][1], color=colorm(i * cm_num),
...
def cor_exp(*args):
    ...
    if type(args[0]) != str:
        ...
        mjd = args[1]
        fiber = str(args[2]).zfill(4)
        path_fits = './new_fits/files/known/spec-%s-%d-%s.fits' % (plate, mjd, fiber)
    else:
        path_fits = '/Users/sarelg/gastro/sdss/files/%s' % args[0]
